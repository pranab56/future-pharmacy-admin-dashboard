...
export default function AllDriverList() {
  ...
  const getStatusBadgeClass = (status: Driver['status']): string => {
    if (status === 'Available') {
            ...
            <tbody>
              {currentData.map((item, index) => (
                <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-6 py-4 text-sm text-gray-900">{item.driverId}</td>
                  <td className="px-6 py-4 text-sm text-gray-900">{item.driverName}</td>
                  <td className="px-6 py-4 text-sm text-gray-900">{item.contact}</td>
